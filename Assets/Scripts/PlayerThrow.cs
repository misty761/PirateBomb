...
public class PlayerThrow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (player.animator.GetCurrentAnimatorStateInfo(0).IsName("DoorOut")) return;
        if (player.animator.GetCurrentAnimatorStateInfo(0).IsName("DoorIn")) return;
        ...
        else
        {
            if (Input.GetKeyDown(KeyCode.DownArrow) || button.isTouchDown)
            {
                ...
                if (timeThrow == 0f)
                {
                    ...
                    SoundManager.instance.PlaySound(SoundManager.instance.audioClick, transform.position, 0.1f);
